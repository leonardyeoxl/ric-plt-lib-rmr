...
static void* init(  char* uproto_port, int max_msg_size, int flags ) {
	...
	ctx->rtable = rt_clone_space( ctx, NULL, NULL, 0 );			// allows wormhole and rts calls to work before rt is received
	if( flags & FL_NOTHREAD ) {								// if no rtc thread, we still need an empty route table for wormholes
		...
		ctx->rmr_ready = 0;									// ensure not ready until static/dynamic table loaded
